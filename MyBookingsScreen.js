        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <View style={styles.card}>
              ...
              <Text>{item.location} • {item.hours}h • ₦{item.totalCost?.toLocaleString()}</Text>
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        data={myBookings}
        keyExtractor={(item) => item.id}
        ListEmptyComponent={<Text style={styles.empty}>No bookings yet — book a driver from Home.</Text>}
        renderItem={({ item }) => (
          <View style={styles.card}>
            <Text style={styles.cardTitle}>{item.carDetails}</Text>
            <Text>{item.location} • {item.hours}h • ₦{item.totalCost?.toLocaleString()}</Text>
